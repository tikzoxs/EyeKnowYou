...
def main():
    ...
    # Define model
    model = Sequential()
    model.add(Conv3D(64, kernel_size=(3, 3, 3), input_shape=(128, 192, 64, 1), border_mode='same', name='new_input'))
    model.add(Activation('relu'))
    model.add(Conv3D(64, kernel_size=(3, 3, 3), border_mode='same')) #, trainable = False
    model.add(Activation('relu'))
    ...
    # model.add(Dropout(0.25))
    model.add(Dense(2, activation='sigmoid', name='new_output'))
    ...
    model.load_weights("checkpoint/eky_weights.hd5")#, by_name=True
